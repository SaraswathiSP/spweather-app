...
const SearchScreen = () => {
  ...
  const toggleShowAllLocations = () => {
    setShowAllLocations((prevShowAllLocations) => !prevShowAllLocations);
  };

  const filteredWeatherData = showAllLocations
    ? weatherData
    : weatherData.filter((data) => data.isFavourite);
    ...
    <div className="search-container">
      ...
      <form className="form" onSubmit={handleSubmit}>
        <button>
          ...
          >
            <path
              d="M7.667 12.667A5.333 5.333 0 107.667 2a5.333 5.333
              0 0 000 10.667zM14.334 14l-2.9.9"
              stroke="currentColor"
              strokeWidth="1.333"
              strokeLinecap="round"
      ...
      <div className="only-favourites">
        <button type="button" className="fav-button" onClick={toggleShowAllLocations}>
          {showAllLocations ? 'Show Favourites' : 'Show All Locations'}
        </button>
      </div>

      <div className="user-favorite-locations">
        {filteredWeatherData.map((each) => (
